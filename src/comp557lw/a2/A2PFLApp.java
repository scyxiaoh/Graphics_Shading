...
public class A2PFLApp{
	...
	// Objective 6: Set the second parameter, default value of sigma, to be as small as possible while avoiding self shadowing
	...
	public void run() {
			// Objective 0: add your name to the window title
			System.out.println("Kevin Sun " + Version.getVersion() + "!");	    
			windowInit();
			init();
			loop();
	...
	public void init() {
		...
		// OBJECTIVE 1: add ambient lighting to file shader/mode1.fp
		// OBJECTIVE 2: add ambient lighting to file shader/mode2.fp
		// OBJECTIVE 3: add ambient lighting to file shader/mode3.fp
		// OBJECTIVE 6: add ambient lighting to file shader/mode4.fp
		programID[0] = 0;
		for ( int i = 1; i <= 4; i++ ) {
	...
	public void display() {
		...
		if (this.lightPosx.getValue() != 0 || this.lightPosz.getValue() != 0) {
			glLoadIdentity();        
			// Objective 4: set up the light viewing transformation
			GLU.gluLookAt(this.lightPosx.getValue(), this.lightPosy.getValue(), this.lightPosz.getValue(), 0.0, 0.0, 0.0, 0, 1, 0);
		...
		glMatrixMode( GL_PROJECTION );
		glLoadIdentity();
		// Objective 5: set up the light projection transformation
		GLU.gluPerspective(lightFOV.getValue(), 1, 2, 17);
		...
		// Objective 6: Build the matrix that transforms from camera frame to the light's NDC
		FlatMatrix4f modelViewInverse = new FlatMatrix4f(); 	
		FlatMatrix4f lightProjection = new FlatMatrix4f(); 
		FlatMatrix4f lightView = new FlatMatrix4f(); 
		
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		tbc.applyInverseViewTransformation(); 
		glGetFloatv( GL_MODELVIEW_MATRIX, modelViewInverse.asArray() );
		modelViewInverse.reconstitute();	
		//System.out.println("inverse " + modelViewInverse.getBackingMatrix());
		
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		GLU.gluPerspective(lightFOV.getValue(), 1, 2, 17);
		glGetFloatv( GL_PROJECTION_MATRIX, lightProjection.asArray() );
		lightProjection.reconstitute();
		//System.out.println("projection " + lightProjection.getBackingMatrix());
		
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		GLU.gluLookAt(this.lightPosx.getValue(), this.lightPosy.getValue(), this.lightPosz.getValue(), 0.0, 0.0, 0.0, 0, 1, 0);
		glGetFloatv( GL_PROJECTION_MATRIX, lightView.asArray() );
		lightView.reconstitute();
		//modelView.getBackingMatrix().invert(modelView.getBackingMatrix());
		//System.out.println("modelview " + lightView.getBackingMatrix());
		
		
		Matrix4f LPM = lightProjection.getBackingMatrix();
		LPM.mul(lightView.getBackingMatrix());
		LPM.mul(modelViewInverse.getBackingMatrix());
		lightProjectionMatrix = new FlatMatrix4f(LPM);
		//System.out.println("final " + lightProjectionMatrix.getBackingMatrix());
		...
		if ( drawFrustum.getValue() ) {
			...
			glPushMatrix();
			// Objective 4: Use the appropriate transformations for drawing the light's camera frame			
			glLightModelfv( GL_LIGHT_MODEL_AMBIENT, new float[] {.5f,.5f,.5f,1});
			glMultMatrixf(this.LightViewingTransformationInverse.asArray());
			...
			glPushMatrix();
			// Objective 5: setup the appropriate matrices to draw the light frustum's NDC
			...
			glMultMatrixf(this.LightViewingTransformationInverse.asArray());
			glMultMatrixf(this.LightProjectionTransformationInverse.asArray());
